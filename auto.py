    ...
    try:
        if 'boosts' in data['data']['product_title'].lower():
            ...
            for i in data['data']['custom_fields']:
                ...
                if i == config['field_name_nick']:
                    nick = data['data']['custom_fields'][i].capitalize()
                if i == config['field_name_bio']:
                    bio = data['data']['custom_fields'][i]
                    
            if nick == "":
                nick = config['server_nick'].capitalize()
            if bio == "":
                bio = config['server_bio'].capitalize()
...
def start_sellapp(data):
    try:
        ...
        for i in data['additional_information']:
            ...
            if i['label'] == config['field_name_nick']:
                nick = i['value'].capitalize()

            if i['label'] == config['field_name_bio']:
                nick = i['value'].capitalize()
...
def start_sellpass(received):
    try:
        ...
        for i in r.json()['data']['partInvoices'][0]['customFields']:
            ...
            if i['customField']['name'] == config['field_name_nick']:
                nick = i['valueString'].capitalize()
        
            if i['customField']['name'] == config['field_name_bio']:
                bio = i['valueString'].capitalize()
        ...
        if product_name.replace(" ", "-").split("-")[0].lower() == "custom":
